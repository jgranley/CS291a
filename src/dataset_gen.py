...
def rand_stim(implant, n_electrodes=1):
    ...
    sample_elecs = random.randint(0, n_electrodes)
...
def rand_percepts(model, implant, n_elecs=1, n_samples=10000):
    ...
    for i in range(n_samples):
        ...
        percept = model._predict_spatial_jax(stim, x, y)
        stims.append(stim)
...
def read_h5(path):
    ...
    stims = hf.get('stims')
    percepts = hf.get('percept')
    hf.close()
    return np.array(percepts), np.array(stims)
...
if __name__ == '__main__':
    ...
    n_electrodes = 1
    n_samples = 6000
    model = p2p.models.BiphasicAxonMapModel(engine='jax', a4=0, xrange=(-14, 12), yrange=(-12, 12), xystep=0.5, axlambda=800)
    model.build()
    implant = p2p.implants.ArgusII(rot=-30)
    percepts, stims = rand_percepts(model, implant)
